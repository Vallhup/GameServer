...
bool RecvBuffer::Write(const BYTE* data, int dataSize)
{
...
bool RecvBuffer::Peek(void* outBuffer, int size) const
{
	...
	std::memcpy(static_cast<BYTE*>(outBuffer) + firstCopySize, &_buffer[0], secondCopySize);

	return true;
}

bool RecvBuffer::Read(BYTE* readBuffer, int readSize)
{
